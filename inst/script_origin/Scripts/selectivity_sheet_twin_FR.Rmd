---
title: "Fiche de sélectivité - InseR"
subtitle: "Protocole en chaluts jumeaux"
output: word_document
---

## Engins testés

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

##Création d'une table contenant le nom des engins, le dispositif sélectitif, le maillage du cul, 
#la longueur de corde de dos

  df<-data %>%
  group_by(gear_label) %>%
  summarise(type=unique(gear_type),sel=unique(selective_device),mesh=unique(mesh_gauge_codend_mm),length=unique(headline_cumulative_length)) %>% as.data.frame()
  
  names(df)<-c("","Engin","Dispositif sélectif","Maillage du cul (mm)","Corde de dos (m)")
  
kable(df)

```

# 1. Description de la marée

### Navire et marées étudiées:

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

##Création d'une table contenant le nom des navires, des marées
#Pour chaque navire x marée: nombre de stations, dates

  df<-data %>%
  group_by(project,vessel_identifier,vessel_name,trip_code) %>%
  summarise(NombreOP=length(unique(station_number)),Debut=unique(departure_date_time),Fin=unique(return_date_time)) %>% as.data.frame()
  

  names(df)<-c("Projet","ID Navire","Navire","ID Marée","Opérations de pêche","Début","Fin")
  
kable(df)

```


### Carte des opérations de pêche
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE,fig.width=30,fig.height=12}

Maps

```

### Conditions de pêche

```{r, echo=FALSE,warning=FALSE,fig.cap="Durées des opérations de pêche en heures.",comment="",message=FALSE,fig.asp=0.5}

tab_OP<- TAB %>% group_by(project,vessel_identifier,trip_code,station_number)%>%
    summarize(fishing_duration=unique(fishing_duration),gear_depth=unique(gear_depth),
              wind_force_beaufort=unique(wind_force_beaufort),gear_speed=unique(gear_speed),sea_state=unique(sea_state))


if(length(which(is.na(tab_OP$fishing_duration)==FALSE))>0){
  ggplot(tab_OP,aes(y=fishing_duration))+
  geom_boxplot(width=0.5)+labs(y="Durée de pêche (heures)")+
  #stat_summary(fun=mean, geom="point", shape=4, lwd=2)+
  geom_point(aes(x=0,y=mean(fishing_duration)),shape=4) + 
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10),axis.text.x=element_blank())+
    xlab("")
}

nNA<-length(which(is.na(tab_OP$fishing_duration)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP), " opérations de pêche."))}
  
```

```{r, echo=FALSE,warning=FALSE,fig.cap="Profondeurs de pêche.",comment="",message=FALSE,fig.asp=0.5}


if(length(which(is.na(tab_OP$gear_depth)==FALSE))>0){
  ggplot(tab_OP,aes(y=gear_depth))+
  geom_boxplot(width=0.5)+labs(y="Profondeur (m)")+
  geom_point(aes(x=0,y=mean(gear_depth)),shape=4) + 
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10),axis.text.x=element_blank())+
    xlab("")
}

nNA<-length(which(is.na(tab_OP$gear_depth)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP), " opérations de pêche."))}
  cat("\n   \n")
  cat("\n   \n") 
```

```{r, echo=FALSE,warning=FALSE,fig.cap="Vitesse de traîne.",comment="",message=FALSE,fig.asp=0.5}

if(length(which(is.na(tab_OP$gear_speed)==FALSE))>0){
  
    ggplot(tab_OP,aes(y=gear_speed))+
  geom_boxplot(width=0.5)+labs(y="Vitesse de pêche (noeuds)")+
  geom_point(aes(x=0,y=mean(gear_speed)),shape=4) + 
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10),axis.text.x=element_blank())+
    xlab("")
  
}

nNA<-length(which(is.na(tab_OP$gear_speed)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP), " opérations de pêche."))}

```
```{r, echo=FALSE,warning=FALSE,fig.cap="Etat de la mer.",comment="",message=FALSE,fig.asp=0.5}

if(length(which(is.na(tab_OP$sea_state)==FALSE))>0){
  
    ggplot(tab_OP,aes(y=sea_state))+
  geom_boxplot(width=0.5)+labs(y="Etat de la mer (Douglas)")+
  geom_point(aes(x=0,y=mean(sea_state)),shape=4) + 
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10),axis.text.x=element_blank())+
    xlab("")
  
}

nNA<-length(which(is.na(tab_OP$gear_speed)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP), " opérations de pêche."))}

```

```{r, echo=FALSE,warning=FALSE,fig.cap="Force du vent.",comment="",message=FALSE,fig.asp=0.5}

if(length(which(is.na(tab_OP$wind_force_beaufort)==FALSE))>0){

      ggplot(tab_OP,aes(y=wind_force_beaufort))+
  geom_boxplot(width=0.5)+labs(y="Force du vent (Beaufort)")+
  geom_point(aes(x=0,y=mean(wind_force_beaufort)),shape=4) + 
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10),axis.text.x=element_blank())+
    xlab("")
}

nNA<-length(which(is.na(tab_OP$wind_force_beaufort)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP), " opérations de pêche."))}


```


```{r, echo=FALSE,warning=FALSE,fig.cap="Type de fond.",comment="",message=FALSE,fig.asp=0.5}

#Conditions de pêche /OP - variables catégorielles

tab_OP2<- TAB %>% group_by(project,vessel_identifier,trip_code,station_number)%>%
    summarize(seabed_features=unique(seabed_features),
              wind_cardinal_direction=unique(wind_cardinal_direction),diurnal_operation=unique(diurnal_operation))


if(length(which(is.na(tab_OP2$seabed_features)))==0){
ggplot(tab_OP2,aes(x=seabed_features))+ geom_bar(stat = 'count',width=0.2) +
 labs(y = "Opérations de pêche",x="Type de substrat")+
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10))
}
nNA<-length(which(is.na(tab_OP2$seabed_features)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP2), " opérations de pêche."))}
  
```

```{r, echo=FALSE,warning=FALSE,fig.cap="Direction du vent.",comment="",message=FALSE,fig.asp=0.5}
if(length(which(is.na(tab_OP2$wind_cardinal_direction)))==0){
ggplot(tab_OP2,aes(x=wind_cardinal_direction))+ geom_bar(stat = 'count',width=0.2) +
 labs(y = "Opérations de pêche",x="Direction du vent")+
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10))
}
nNA<-length(which(is.na(tab_OP2$wind_cardinal_direction)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP2), " opérations de pêche."))}

```


```{r, echo=FALSE,warning=FALSE,fig.cap="Occurences jour/nuit.",comment="",message=FALSE,fig.asp=0.5}
if(length(which(is.na(tab_OP2$diurnal_operation)))==0){
tab_OP2$diurnal_operation<-factor(tab_OP2$diurnal_operation,levels=c("Y","N"),labels=c("Jour","Nuit"))

ggplot(tab_OP2,aes(x=diurnal_operation))+ geom_bar(stat = 'count',width=0.2) +
 labs(y = "Opérations de pêche",x="")+
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10))
}

nNA<-length(which(is.na(tab_OP2$diurnal_operation)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_OP2), " opérations de pêche."))}

  
```



### Résumé des conditions de pêche 

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

varC<-c("fishing_duration","gear_depth","gear_speed","wind_force_beaufort","sea_state")

df<-data.frame(Variable=c("Durée de pêche","Profondeur de pêche","Vitesse de pêche","Force du vent","Etat de la mer"))  
df$min<-apply(tab_OP[,varC],2,"min")
df$max<-apply(tab_OP[,varC],2,"max")
df$mean<-apply(tab_OP[,varC],2,"mean")
df$median<-apply(tab_OP[,varC],2,"median")

df$diff<-0

df[,-1]<-round(df[,-1],1)

df$nNA<-apply(tab_OP[,varC],2,function(x){length(which(is.na(x)==TRUE))})


# #Pour la capture totale et le temps de tri
# var<-c("catch_weight","sorting_time")
# 
# df2<-data.frame(Variable=c("Poids de la capture (kg)","Durée de tri (min)"))  
# df2$min<-apply(tab_capture[,var],2,"min",na.rm=T)
# df2$max<-apply(tab_capture[,var],2,"max",na.rm=T)
# df2$mean<-apply(tab_capture[,var],2,"mean",na.rm=T)
# df2$median<-apply(tab_capture[,var],2,"median",na.rm=T)
# 
# 
# df2$diff<-data.frame(tab_capture) %>%
#   summarize(catch_weight=mean(catch_weight[gear_label=="TEST"]-catch_weight[gear_label=="STD"],na.rm=T),
#             sorting_time=mean(sorting_time[gear_label=="TEST"]-sorting_time[gear_label=="STD"],na.rm=T))%>%as.numeric()
# 
# df2[,-1]<-round(df2[,-1],1)
# df2$nNA<-apply(tab_capture[,var],2,function(x){length(which(is.na(x)==TRUE))})
# 
# df<-rbind(df,df2)

if(sum(df$nNA>0)){
names(df)<-c("","Min","Max","Moyenne","Médiane","Différence","Valeurs NA")

kable(df)
}else{df<-select(df,-nNA)
names(df)<-c("","Min","Max","Moyenne","Médiane","Différence")
kable(df)
}

```



# 2. Analyse des captures

## 2.1. Capture totale

### Poids des captures

```{r, echo=FALSE,warning=FALSE,fig.cap="Poids des captures.",comment="",message=FALSE,fig.asp=0.5}

#Comparaison des poids de captures, temps de tri, poids des rejets / OP :

##Calcul de la durée de tri
TAB$sorting_start_date_time<-as.POSIXct(TAB$sorting_start_date_time,format="%Y-%m-%d %H:%M:%S")
TAB$sorting_end_date_time<-as.POSIXct(TAB$sorting_end_date_time,format="%Y-%m-%d %H:%M:%S")
TAB$sorting_time<-with(TAB,difftime(sorting_end_date_time,sorting_start_date_time,units="mins"))

tab_capture<- TAB %>% 
  group_by(project,vessel_identifier,trip_code,station_number,gear_label)%>%
  summarize(catch_weight=unique(catch_weight),sorting_time=unique(sorting_time),
              discard_weight=unique(discard_weight))

if(length(which(is.na(tab_capture$catch_weight)==FALSE))>0){
  
  ggplot(tab_capture,aes(y=catch_weight,x=gear_label))+
  geom_boxplot(width=0.5,aes(fill=gear_label))+labs(y="Poids de la capture (kg)")+
   stat_summary(fun=mean, geom="point", shape=4, position = position_dodge(0.75),lwd=2)+
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10))+
    xlab("")
}
nNA<-length(which(is.na(tab_capture$catch_weight)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_capture), " opérations de pêche."))}

```

### Durée du tri

```{r, echo=FALSE,warning=FALSE,fig.cap="Durée du tri de la capture par opération de pêche.",comment="",message=FALSE,fig.asp=0.5}

if(length(which(is.na(tab_capture$sorting_time)==FALSE))>0){
  
  ggplot(tab_capture,aes(y=sorting_time,x=gear_label))+
  geom_boxplot(width=0.5,aes(fill=gear_label))+labs(y="Durée du tri (minutes)")+
   stat_summary(fun=mean, geom="point", shape=4, position = position_dodge(0.75),lwd=2)+
  theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10))+
    xlab("")
}
nNA<-length(which(is.na(tab_capture$sorting_time)==T))
if(nNA>0){cat(paste0("Note : il manque ",nNA," valeurs sur les ", nrow(tab_capture), " opérations de pêche."))}


  ## Test sur temps de tri?

```


```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

#Pour chaque OP et fraction, difference de biomasse TEST-STD:

tab_diff<-tab_capture %>% group_by(project,vessel_identifier,trip_code,station_number)%>%
  summarize(weight_DIS_STD=sorting_time[gear_label=="STD"],weight_DIS_TEST=sorting_time[gear_label=="TEST"],
            diff_DIS=sorting_time[gear_label=="TEST"]-sorting_time[gear_label=="STD"])

diff_DIS<-tab_diff$diff_DIS

#Test de Shapiro-Wilk de normalité
#Encapsulage des erreurs: 

test1<-tryCatch(shapiro.test(diff_DIS), error = function(e){"e"})

#if pas "erreur":
if(class(test1) =="htest"){
  
  #Si normalité non rejetée (p-value >0.05 du test de Shapiro), test de Student:
  if(test1$p.value > 0.05){
    test2<-tryCatch(t.test(x=diff_DIS,mu=0) , error = function(e){"e"})
  }
  #Si normalité rejetée (p-value <= 0.05 du test de Shapiro), test de Mann-Whitney/Wilcoxon:
  if(test1$p.value <= 0.05){
    test2<-tryCatch(wilcox.test(diff_DIS, paired =F, correct = T, exact = F), error = function(e){"e"})
  }
  
  if(class(test2) == "htest"){#si pas d'erreur
    
    if(test2$p.value<=0.05){
      
      cat("Les durées de tri sont significativement différentes entre l'engin test et le standard.","\n")
 }else{ cat("Les durées de tri ne sont pas significativement différentes entre l'engin test et le standard.","\n")}
      
  }    
      
  if(test1$p.value <= 0.05){
        cat(paste("Il y a en médiane",abs(round(median(diff_DIS),3)),"minutes de",c("moins","plus")[as.numeric(eval(median(diff_DIS)>0))+1],"dans l'engin test comparé à l'engin standard (p-value=",round(test2$p.value,4),")."))  
      }
  
  if(test1$p.value > 0.05){
      cat(paste("Il y a en moyenne",abs(round(mean(diff_DIS),3)),"minutes de",c("moins","plus")[as.numeric(eval(mean(diff_DIS)>0))+1],"dans l'engin test comparé à l'engin standard (p-value=",round(test2$p.value,4),")."))  
      }
   
}

```


### Taux de variation des durées de tri pour l'engin test par rapport à l'engin standard

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

#Taux de variation (TEST-STD)/STD
Tot_TEST<-sum(tab_diff$weight_DIS_TEST)
Tot_STD<-sum(tab_diff$weight_DIS_STD)

Var_Rate<-(Tot_TEST-Tot_STD)/Tot_STD*100

if(is.na(Var_Rate)==FALSE){
cat(paste("Une",c("diminution" ,"augmentation")[as.numeric(eval(Var_Rate>0))+1],"de",abs(round(Var_Rate,2)),"% de la durée de tri est observée avec l'engin test en considérant la durée sur l’ensemble des des OPs suivies."))

Var_Rate2<-with(tab_diff,diff_DIS/weight_DIS_STD)*100
  
cat(paste("Une",c("diminution" ,"augmentation")[as.numeric(eval(mean(Var_Rate2)>0))+1],"de",abs(round(mean(Var_Rate2),2)),"% de la durée de tri est observée avec l'engin test en moyennnant la durée par OP."))
}
```


### Poids des rejets

```{r, echo=FALSE,warning=FALSE,message=FALSE,fig.width=3}

if(length(which(is.na(tab_capture$discard_weight)))==0){
  ggplot(tab_capture,aes(y=discard_weight,x=gear_label,fill=gear_label))+
  geom_boxplot(width=0.5)+labs(y = "Poids des rejets par capture (kg)")+
  xlab("Engin")+
    theme_light()+
    theme(legend.position = "None",axis.text=element_text(size=8),
        axis.title=element_text(size=10))+
  stat_summary(fun=mean, geom="point", shape=4, position = position_dodge(0.75),lwd=2)
}

# ##Comparaison des poids pêchés sur le total de la liste
# 
# #Table des poids pour chaque Num_Trait, Engin, Fraction:
# tab_catch_list<-TAB%>%
#   group_by(project,vessel_identifier,trip_code,station_number,gear_label,catch_category)%>%
#   summarize(weight=sum(weight,na.rm=T))%>% as.data.frame()
# 
# tab_catch_list<-as.data.frame(complete(tab_catch_list,nesting(project,vessel_identifier,trip_code,station_number),gear_label,catch_category, fill = list(weight = 0)))
# 
# ##Boxplots du poids des rejets 
# ggplot(filter(tab_catch_list,catch_category=="DIS"),aes(y=weight,x=gear_label,fill=gear_label))+
#   geom_boxplot(width=0.5)+labs(y = "Poids des rejets ciblés par capture (Kg)")+
#   theme(legend.position = "None",axis.text=element_text(size=12),
#         axis.title=element_text(size=14))+
#   stat_summary(fun=mean, geom="point", shape=4, position = position_dodge(0.75),lwd=2)


```


```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

#Pour chaque OP et fraction, difference de biomasse TEST-STD:

tab_diff<-tab_capture %>% group_by(project,vessel_identifier,trip_code,station_number)%>%
  summarize(weight_DIS_STD=discard_weight[gear_label=="STD"],weight_DIS_TEST=discard_weight[gear_label=="TEST"],
            diff_DIS=discard_weight[gear_label=="TEST"]-discard_weight[gear_label=="STD"])

diff_DIS<-tab_diff$diff_DIS

#Test de Shapiro-Wilk de normalité
#Encapsulage des erreurs: 

test1<-tryCatch(shapiro.test(diff_DIS), error = function(e){"e"})

#if pas "erreur":
if(class(test1) =="htest"){
  
  #Si normalité non rejetée (p-value >0.05 du test de Shapiro), test de Student:
  if(test1$p.value > 0.05){
    test2<-tryCatch(t.test(x=diff_DIS,mu=0) , error = function(e){"e"})
  }
  #Si normalité rejetée (p-value <= 0.05 du test de Shapiro), test de Mann-Whitney/Wilcoxon:
  if(test1$p.value <= 0.05){
    test2<-tryCatch(wilcox.test(diff_DIS, paired =F, correct = T, exact = F), error = function(e){"e"})
  }
  
  if(class(test2) == "htest"){#si pas d'erreur
    
    if(test2$p.value<=0.05){
      
      cat("Le poids des rejets totaux sont significativement différents entre l'engin test et le standard.","\n")
 }else{ cat("Les poids rejetés ne sont pas significativement différents entre l'engin test et le standard.","\n")}
      
  }    
      
  if(test1$p.value <= 0.05){
        cat(paste("Il y a en médiane",abs(round(median(diff_DIS),3)),"kg de",c("moins","plus")[as.numeric(eval(median(diff_DIS)>0))+1],"dans l'engin test comparé à l'engin standard (p-value=",round(test2$p.value,4),")."))  
      }
  
  if(test1$p.value > 0.05){
      cat(paste("Il y a en moyenne",abs(round(mean(diff_DIS),3)),"kg de",c("moins","plus")[as.numeric(eval(mean(diff_DIS)>0))+1],"dans l'engin test comparé à l'engin standard (p-value=",round(test2$p.value,4),")."))  
      }
   
}

```


### Taux de variation des poids rejetés totaux de l'engin test par rapport à l'engin standard

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

#Taux de variation (TEST-STD)/STD
Tot_TEST<-sum(tab_diff$weight_DIS_TEST)
Tot_STD<-sum(tab_diff$weight_DIS_STD)

Var_Rate<-(Tot_TEST-Tot_STD)/Tot_STD*100

cat(paste("Une",c("diminution" ,"augmentation")[as.numeric(eval(Var_Rate>0))+1],"de",abs(round(Var_Rate,2)) ,"% de rejets en poids est observée avec l'engin test en considérant le poids de l’ensemble des rejets des OPs suivies."))

Var_Rate2<-with(tab_diff,diff_DIS/weight_DIS_STD)*100
  
cat(paste("Une",c("diminution" ,"augmentation")[as.numeric(eval(mean(Var_Rate2)>0))+1],"de",abs(round(mean(Var_Rate2),2)),"% en poids des rejets est observée avec l'engin test en moyennnant la perte par OP."))

```



## 2.2. Comparaison des poids débarqués et rejetés par espèce étudiée

### Liste des espèces débarquées étudiées :
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
List_LAN<-unique(filter(TAB,catch_category=="LAN")$species)
cat(List_LAN,sep=", ")

```

### Liste des espèces rejetées étudiées :
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
List_DIS<-unique(filter(TAB,catch_category=="DIS")$species)
cat(List_DIS,sep=", ")
  cat("\n   \n")
  cat("\n   \n") 
```

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
List_sp<-unique(TAB$species)

#Boxplot des poids débarqués de chaque fraction / OP: 
weight_species<-TAB %>% group_by(project,vessel_identifier,trip_code,station_number,gear_label,catch_category,species)%>%
 summarize(weight=sum(weight)*10^(-3))%>%as.data.frame()

weight_species<-as.data.frame(complete(weight_species,nesting(project,vessel_identifier,trip_code,station_number),gear_label,catch_category,species,fill=list(weight=0)))

#Un graphe par espece
List_ggplot<-vector(mode="list",length=length(List_sp))

for(sp in 1:length(List_sp)){
  
  List_ggplot[[sp]]<-ggplot(filter(weight_species,species==List_sp[sp]),aes(y=weight,x=catch_category,fill=gear_label))+
    geom_boxplot()+labs(y = "Poids par capture (Kg)")+ggtitle(List_sp[sp]) +
    stat_summary(fun=mean, geom="point", shape=4, position = position_dodge(0.75),lwd=2)+
    xlab("")+
    theme_light()+
    scale_x_discrete(labels=c("Rejets","Débarquements"))+
    #facet_wrap(~species, scales="free") + 
    labs(fill = "Engin")
  print(List_ggplot[[sp]])
}

```


```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}


##Pour chaque espèce et fraction, test apparié (STD/TEST) sur les différences de poids par capture
tab_diff<-weight_species %>%group_by(project,vessel_identifier,trip_code,station_number,catch_category,species)%>%
  summarize(diff_weight=weight[gear_label=="TEST"]-weight[gear_label=="STD"],
            weight_STD=weight[gear_label=="STD"],weight_TEST=weight[gear_label=="TEST"])

Ftest<-function(sp,cat){
  
  #différence de poids de la fraction cat par paire de l'espèce sp
  Diff_sp<-filter(tab_diff,species==sp & catch_category==cat)  
  
  #1.Test de Shapiro-Wilk de normalité
  test1<-tryCatch(shapiro.test(Diff_sp$diff_weight),error=function(e){"e"})    
  
  if(class(test1) =="htest"){
    
    #Si normalité non rejetée (p-value >0.05 du test de Shapiro), test de Student:
    if(test1$p.value > 0.05){
      test2<-tryCatch(t.test(x=Diff_sp$diff_weight,mu=0),error=function(e){"e"})   
    }
      
    #Si normalité rejetée (p-value <= 0.05 du test de Shapiro), test de Mann-Whitney/Wilcoxon:
    if(test1$p.value <= 0.05){
      test2<-tryCatch(wilcox.test(Diff_sp$diff_weight, paired =F, correct = T, exact = F),error=function(e){"e"})  
    }
      
    ##Resultat du test de Student/Wilcoxon: 
    if(test1$p.value <= 0.05){  
     tab_test<-data.frame(pvalue=round(test2$p.value,4),moyenne=mean(Diff_sp$diff_weight),mediane=median(Diff_sp$diff_weight),test="Wilcoxon")
    }
  
    if(test1$p.value > 0.05){
      tab_test<-data.frame(pvalue=round(test2$p.value,4),moyenne=mean(Diff_sp$diff_weight),mediane=median(Diff_sp$diff_weight),test="Fisher")
    }
  
  }else{
    tab_test<-data.frame(pvalue=NA,moyenne=mean(Diff_sp$diff_weight),mediane=median(Diff_sp$diff_weight),test=NA)
  }
  
  
  #Taux de variation des poids débarqués du TEST par rapport au STD
  Tot_TEST<-sum(Diff_sp$weight_TEST)
  Tot_STD<-sum(Diff_sp$weight_STD)

  Taux_sp<-round(100*(Tot_TEST-Tot_STD)/Tot_STD,2)#sur le total des marées
  
  Var_Rate2<-with(Diff_sp,diff_weight/weight_STD)*100
  Taux_sp_OP<-round(mean(Var_Rate2,na.rm=T),2)#en moyenne par OP")
  
  tab_test<-data.frame(tab_test, Taux_Var_Tot=Taux_sp, Taux_Var_OP=Taux_sp_OP)
  return(tab_test)
}

test_LAN<-matrix(NA,ncol=6,nrow=length(List_LAN))
row.names(test_LAN)<-List_LAN
test_LAN<-as.data.frame(test_LAN)

for(ii in 1:length(List_LAN)){
  
  #Test apparié:  (H0) Diff_Poids=0 versus (H1) Diff_Poids non nulle sur médiane ou moyenne
  #+ Taux de variation TEST/STD
  test_LAN[ii,]<-Ftest(List_LAN[ii],"LAN")
}
names(test_LAN)<-names(Ftest(List_LAN[ii],"LAN"))


test_LAN$moyenne<-round(test_LAN$moyenne,2)
test_LAN$mediane<-round(test_LAN$mediane,2)

test_LAN$moyenne[test_LAN$test=="Fisher"]<-paste(test_LAN$moyenne[test_LAN$test=="Fisher"],"*",sep="")
test_LAN$mediane[test_LAN$test=="Kolmogorov"]<-paste(test_LAN$moyenne[test_LAN$test=="Kolmogorov"],"*",sep="")

test_LAN$pvalue[test_LAN$pvalue< 10^(-2)]<-"< 10-2"
test_LAN$pvalue[test_LAN$pvalue>0.1]<-">0.1"

names(test_LAN)<-c("p-value","Différence moyenne (kg)","Différence médiane (kg)","Test","Taux de variation total (%)","Taux de variation moyen / OP (%)")

kable(test_LAN[,-c(4)],digits=2,caption="Résultats des tests de différence de poids débarqués pour chaque espèce. La statistique testée est identifiée par '*'. Plus la p-value est faible, plus la différence est significative.")


```

*Rejets*
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

test_DIS<-matrix(NA,ncol=6,nrow=length(List_DIS))
row.names(test_DIS)<-List_DIS
test_DIS<-as.data.frame(test_DIS)

for(ii in 1:length(List_DIS)){
  #Test apparié:  (H0) Diff_Poids=0 versus (H1) Diff_Poids non nulle sur médiane ou moyenne
  #+ Taux de variation TEST/STD
  test_DIS[ii,]<-Ftest(List_DIS[ii],"DIS")
}

names(test_DIS)<-names(Ftest(List_DIS[ii],"DIS"))

# kable(test_PR[,-4],digits=2,caption="Résultats des tests de différence de biomasses débarquées et rejetées pour chaque espèce. Si la distribution était gaussienne, un test de Fisher sur la moyenne a été effectué, sinon un test de Kolmogorov-Smirnov sur la médiane a été effectué. La statistique testée est identifiée par '*'.")

test_DIS$pvalue[test_DIS$pvalue< 10^(-2)]<-"< 10-2"
test_DIS$pvalue[test_DIS$pvalue>0.1]<-">0.1"

test_DIS$moyenne<-round(test_DIS$moyenne,2)
test_DIS$mediane<-round(test_DIS$mediane,2)

test_DIS$moyenne[which(test_DIS$test=="Fisher")]<-paste(test_DIS$moyenne[which(test_DIS$test=="Fisher")],"*",sep="")
test_DIS$mediane[which(test_DIS$test=="Kolmogorov")]<-paste(test_DIS$moyenne[which(test_DIS$test=="Kolmogorov")],"*",sep="")

names(test_DIS)<-c("p-value","Différence moyenne (kg)","Différence médiane (kg)","Test","Taux de variation total (%)","Taux de variation moyen / OP (%)")

kable(test_DIS[,-c(4)],digits=2,caption="Résultats des tests de différence de poids rejetés pour chaque espèce. La statistique testée est identifiée par '*'.")

```



# 3. Etude de la sélectivité en taille

### Liste des espèces étudiées :
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}

List_M<-unique(filter(TAB,is.na(number_at_length)==FALSE)$species)
cat(List_M,sep=", ")

```

### Pour chaque espèce, distribution en tailles des débarquements/rejets par engin

```{r, echo=FALSE,message=FALSE,warning=FALSE,comment="",message=FALSE,fig.width=18,fig.height=6}
  


List_E<-vector(mode="list",length=length(List_M))

for(sp in 1:length(List_M)){
  
  ##Bornes des tailles pour les graphes
  Taille_min<-min(filter(TAB,species==List_M[sp])$length_class)
  Taille_max<-max(filter(TAB,species==List_M[sp])$length_class)
   
  #Titre en abscisse
  unit<-unique(filter(TAB,species==List_M[sp])$length_code)
  type<-unique(filter(TAB,species==List_M[sp])$measure_type)
  
  if(type=="CL" | type=="LC"){
   Nom_Taille<-paste0("Longeur céphalothoracique (", unit,")")  
  }
  if(type=="TL" | type=="LT"){
   Nom_Taille<-paste0("Longeur totale (", unit,")")  
  }
  
  Taille_com<-filter(min_length,species==List_M[sp])$min_length
  
  #Effectifs en taille dans les captures
  length_ALL<-filter(TAB,species==List_M[sp]) %>%
  group_by(gear_label,length_class)%>%
    summarize(n=sum(elevated_number_at_length,na.rm=T))
  
  p1<-ggplot(data=length_ALL, aes(x=length_class, fill=gear_label)) +
      xlim(Taille_min,Taille_max)+
      geom_bar(stat='identity',aes(y=n),position = 'dodge')+
      theme_light()+
    labs(y = "Nombre d'individus",x=Nom_Taille)+
     #facet_wrap(~Fraction, scales="fixed",ncol=3)+ # 1 plot par fraction
      geom_vline(xintercept=Taille_com,color="yellowgreen",size=1,linetype = "dashed")+
    theme(legend.position="bottom",legend.title = element_text(size=22),legend.text = element_text(size=22),axis.text=element_text(size=22),
        axis.title=element_text(size=24),plot.title = element_text(size = 26))+
        labs(fill="Engin")+
    ylim(c(0,max(length_ALL$n)))+
   #ggtitle(paste0(List_M[sp]," (Total)"))  
    ggtitle("Total")
   
  #print(p1)
  
  #Effectifs en taille par fraction
  length_cat<-filter(TAB,species==List_M[sp]) %>%
  group_by(gear_label,length_class,catch_category)%>%
    summarize(n=sum(elevated_number_at_length,na.rm=T))
  
  p2<-ggplot(data=filter(length_cat,catch_category=="LAN"), aes(x=length_class, fill=gear_label)) +
      xlim(Taille_min,Taille_max)+
      geom_bar(stat='identity',aes(y=n),position = 'dodge')+
      theme_light()+
    labs(y = "Nombre d'individus",x=Nom_Taille)+
      geom_vline(xintercept=Taille_com,color="yellowgreen",size=1,linetype = "dashed")+
          theme(legend.position="bottom",legend.title = element_text(size=22),legend.text = element_text(size=22),axis.text=element_text(size=22),
        axis.title=element_text(size=24),plot.title = element_text(size = 26))+
        labs(fill="Engin")+
        ylim(c(0,max(length_ALL$n)))+
     #ggtitle(paste0(List_M[sp]," (Débarquements)"))
        ggtitle("Débarquements")
  #print(p2)
  
  p3<-ggplot(data=filter(length_cat,catch_category=="DIS"), aes(x=length_class, fill=gear_label)) +
      xlim(Taille_min,Taille_max)+
      geom_bar(stat='identity',aes(y=n),position = 'dodge')+
      theme_light()+
    labs(y = "Nombre d'individus",x=Nom_Taille)+
      geom_vline(xintercept=Taille_com,color="yellowgreen",size=1,linetype = "dashed")+
     theme(legend.position="bottom",legend.title = element_text(size=22),legend.text = element_text(size=22),axis.text=element_text(size=22),
        axis.title=element_text(size=24),plot.title = element_text(size = 26))+
    labs(fill="Engin")+
        ylim(c(0,max(length_ALL$n)))+
        ggtitle("Rejets")
  #   ggtitle(paste0(List_M[sp]," (Rejets)"))
  #print(p3)
  
  
  p<-ggarrange(p1,p2,p3,nrow=1,common.legend = T)
  
  cat(List_M[sp])

  print(p)

  ##Liste par espèce des taux d'échappements (STD-SEL)/STD: 
  #sur toute la capture, HT et taille com.
  
  nSTD<-sum(filter(length_ALL,gear_label=="STD")$n,na.rm=T)
  nSEL<-sum(filter(length_ALL,gear_label=="TEST")$n,na.rm=T)
  taux_Total<-(nSTD-nSEL)/nSTD
  
  nSTD<-sum(filter(length_ALL,gear_label=="STD" & length_class >= Taille_com)$n,na.rm=T)
  nSEL<-sum(filter(length_ALL,gear_label=="TEST" & length_class >= Taille_com)$n,na.rm=T)
  taux_com<-(nSTD-nSEL)/nSTD
  
  nSTD<-sum(filter(length_ALL,gear_label=="STD" & length_class < Taille_com)$n,na.rm=T)
  nSEL<-sum(filter(length_ALL,gear_label=="TEST" & length_class < Taille_com)$n,na.rm=T)
  taux_HT<-(nSTD-nSEL)/nSTD
  
  tab_E<-data.frame(Espece=List_M[sp],taux_Total*100,taux_com*100,taux_HT*100)
  
  List_E[[sp]]<-tab_E

}


Tab_E<-bind_rows(List_E)

names(Tab_E)<-c("Espece","Global (toutes les tailles)","Taille com.","Hors taille")

kable(Tab_E,digits=2,caption="Taux d'échappement de l'engin test par rapport au standard.")

```


### Courbes de sélectivité relative du chalut test par rapport le chalut standard

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE,fig.asp=0.7}

for(sp in 1:length(List_M)){
  

  length_sp<-filter(TAB,species==List_M[sp]) %>% group_by(gear_label,length_class) %>%
  summarize(n=sum(elevated_number_at_length,na.rm=T)) %>%as.data.frame()
  
  #sel relative = TEST/TOTAL
  length_sp<-complete(length_sp,gear_label,length_class,fill=list(n=0))

  Rel_Sel<-length_sp %>% group_by(length_class)%>%
    summarize(rel_sel=n[gear_label=="TEST"]/sum(n),n_STD=sum(n[gear_label=="STD"]),n_TEST=sum(n[gear_label=="TEST"]))%>%as.data.frame()

  taille_com<-filter(min_length,species==List_M[sp])$min_length
  
   #Titre en abscisse
  unit<-unique(filter(TAB,species==List_M[sp])$length_code)
  type<-unique(filter(TAB,species==List_M[sp])$measure_type)

   if(type=="CL" | type=="LC"){
   Nom_Taille<-paste0("Longueur céphalothoracique (", unit,")")  
   }
   if(type=="TL" | type=="LT"){
   Nom_Taille<-paste0("Longueur totale (", unit,")")  
   }
  
 p<-ggplot(data=Rel_Sel)+
    geom_point(aes(x=length_class,y=rel_sel,size=n_STD+n_TEST))+
     guides(size=guide_legend(title = "n"))+
    xlab(Nom_Taille)+ylab("Sélectivité relative")+
   geom_hline(yintercept=0.5, linetype="dashed", color = "grey")+
     geom_vline(xintercept=taille_com,size=1, linetype="dashed", color = "green")+
    ggtitle(List_M[sp])
  
  print(p)
  
}

```