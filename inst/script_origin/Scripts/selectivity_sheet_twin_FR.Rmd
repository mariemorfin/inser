...
for(ii in 1:length(List_LAN)){
  ...
  test_LAN[ii,]<-f_test(tab_diff,List_LAN[ii],"LAN")
}
names(test_LAN)<-names(f_test(tab_diff,List_LAN[ii],"LAN"))
...
for(ii in 1:length(List_DIS)){
  #Test apparié:  (H0) Diff_Poids=0 versus (H1) Diff_Poids non nulle sur médiane ou moyenne
  #+ Taux de variation TEST/STD
  test_DIS[ii,]<-f_test(tab_diff,List_DIS[ii],"DIS")
}

names(test_DIS)<-names(f_test(tab_diff,List_DIS[ii],"DIS"))
