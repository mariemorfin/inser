...
subtitle: "Protocole en traits alternés"
...
```{r, echo=FALSE,warning=FALSE,fig.cap="Durées de pêche en heures. A gauche : Boxplot pour chaque engin. A droite : Durée de pêche au sein de chaque paires d'opérations de pêche.",comment="",message=FALSE,fig.asp=0.5}

tab_OP<- TAB %>% group_by(project,vessel_identifier,trip_code,tag_operation,gear_label)%>%
...
```{r, echo=FALSE,warning=FALSE,fig.cap="Vitesse de l'engin.",comment="",message=FALSE,fig.asp=0.5}
...
TAB$diurnal_operation<-factor(TAB$diurnal_operation,levels=c("Y","N"),labels=c("Jour","Nuit"))

#Conditions de pêche /OP - variables catégorielles
tab_OP2<- TAB %>% group_by(project,vessel_identifier,trip_code,tag_operation,gear_label)%>%
...
tab_diff<-tab_capture %>% group_by(project,vessel_identifier,trip_code,tag_operation)%>%
...
#Pour chaque paire, difference de rejets TEST-STD:
...
if(class(test1) =="htest"){
  ...
  if(class(test2) == "htest"){#si pas d'erreur
    ...
    if(test2$p.value<=0.05){
      ...
      cat("Le poids des rejets totaux sont significativement différents entre le chalut standard et le chalut sélectif.")
 }else{ cat("Les poids rejetés ne sont pas significativement différents entre l'engin sélectif et le standard.")}
      
  }    
      
  if(test1$p.value <= 0.05){
        cat(paste("Il y a en médiane",abs(round(median(diff_DIS),3)),"kg de",c("moins","plus")[as.numeric(eval(median(diff_DIS)>0))+1],"dans le chalut sélectif comparé au chalut standard (p-value=",round(test2$p.value,4),")."))  
      }
  
  if(test1$p.value > 0.05){
       cat(paste("Il y a en moyenne",abs(round(mean(diff_DIS),3)),"kg de",c("moins","plus")[as.numeric(eval(mean(diff_DIS)>0))+1],"dans le chalut sélectif comparé au chalut standard (p-value=",round(test2$p.value,4),")."))  
      }
...
### Taux de variation des poids rejetés totaux du chalut sélectif par rapport au chalut standard
...
cat(paste("Une perte de",round(Var_Rate,2) ,"% de rejets en poids est observée dans le chalut sélectif en considérant le poids de l’ensemble des rejets des OPs suivies."))

Var_Rate2<-with(tab_tag,(discard_weight_TEST-discard_weight_STD)/discard_weight_STD)*100
  
cat(paste("Une perte de",round(mean(Var_Rate2),2),"% en poids des rejets est observée dans le chalut sélectif en moyennnant la perte par paire d'opération de pêche."))
...
weight_species<-TAB %>% group_by(project,vessel_identifier,trip_code,tag_operation,gear_label,catch_category,species)%>%
 summarize(weight=sum(weight))%>%as.data.frame()
...
names(test_DIS)<-names(Ftest(List_DIS[ii],"LAN"))
