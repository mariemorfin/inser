...
tab_OP<- TAB %>% group_by(project,vessel_identifier,trip_code,station_number,gear_label)%>%
...
tab_OP2<- TAB %>% group_by(project,vessel_identifier,trip_code,station_number,gear_label)%>%
...
Ftest<-function(sp,cat){
  ...
  weight_sp_frac<-filter(weight_species,species==sp & catch_category==cat)
   ...
   #Conditions du test de Student: normalité des 2 échantillons + égalité des variances
  ...
  test1_STD<-tryCatch(shapiro.test(weight_STD),error=function(e){"e"})  
  test1_TEST<-tryCatch(shapiro.test(weight_TEST),error=function(e){"e"})  
  
  #2.Test de Fisher d'égalité des variances:
  test1b<-tryCatch(var.test(weight_STD,weight_TEST),error=function(e){"e"})  

  if(class(test1_STD) =="htest" & class(test1_TEST) =="htest" & class(test1b) =="htest"){
      
    #Si les 2 tests non rejetés (p-value >0.05)=> test de Student:
    if(test1_TEST$p.value > 0.05 & test1_STD$p.value > 0.05 & test1b$p.value > 0.05){
      test2<-tryCatch(t.test(x=weight_STD,y=weight_TEST) ,error=function(e){"e"})   
    }else{
      #Sinon test de Mann-Whitney/Wilcoxon:
      test2<-tryCatch(wilcox.test(weight_STD,weight_TEST, paired =F, correct = T, exact = F),error=function(e){"e"})  
    }
  }
  
  ##Resultat du test: 
  if(class(test2) =="htest"){
  if(test1_TEST$p.value > 0.05 & test1_STD$p.value > 0.05 & test1b$p.value > 0.05){
    tab_test<-data.frame(pvalue=round(test2$p.value,4),moyenne=mean(weight_TEST)-mean(weight_STD),mediane=median(weight_TEST)-median(weight_STD),test="Fisher")
  }else{
    tab_test<-data.frame(pvalue=round(test2$p.value,4),moyenne=mean(weight_TEST)-mean(weight_STD),mediane=median(weight_TEST)-median(weight_STD),test="Kolmogorov")
  }
  
  }else{
    tab_test<-data.frame(pvalue=NA,moyenne=mean(weight_TEST)-mean(weight_STD),mediane=median(weight_TEST)-median(weight_STD),test=NA)
  }
