---
title: "Dev history - Déployer une app Shiny sur Connect"
output: 
  html_document:
    toc: true
    toc_depth: 1
...
Dans ce document, vous trouverez les étapes/commandes importantes pour déployer une app Shiny sur Connect avec le CI.
...
```{r}
# Définir ici le nom de votre app (le suffixe "-app" sera automatiquement ajouté)
name_app <- "myapp"
```
...
rsconnect::deployApp(
  ...
  appName = paste0(name_app, "-app"),
  appFiles = appFiles,          # the list of files to include as dependencies (all of them)
  appPrimaryDoc = "app.R", # the primary file for shinyapp
  # appId = <id-sur-connect>, # Define when known
...
- Ajouter la ligne suivante dans votre gitlab-ci.yml, **après la création du pkgdown**
 ...
 `        - Rscript "dev/deploy_connect_app.R"`
...
Ne pas oublier d'ajouter le stage "deploy-app-connect à la liste des stages du début du fichier gitlab-ci.yml
Dans cet ordre:
...
```{r, eval=FALSE}
knitr::purl("dev/dev_history_connect_app.Rmd", "dev/deploy_connect_app.R")
```
